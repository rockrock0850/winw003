...
<%@ include file="/WEB-INF/jsp/layout/header.jsp"%>	
	<h1><s:message code="form.search.h1.title" text='表單查詢' /></h1>
	<%-- 表單類型欄位查詢 --%>
	<fieldset id="search" style="width: 25%; float: left; margin-left: 20px; margin-right: 20px;">
		<legend>
			...
			<jsp:include page='/WEB-INF/jsp/common/models/selectorModel.jsp'>
				<jsp:param name="id" value="formClass" />
				<jsp:param name="name" value="formClass" />
			</jsp:include>
		...
		<div style="height: 28rem; width: 28rem; overflow: auto;">
			<table id="fieldsTable" class="grid_query">
			</table>
		</div>
	</fieldset>
...
<script>//# sourceURL=formSearchFind.js
var dataTable;
...
function initView () {
	<%-- 取得表單類別清單 --%>
	SendUtil.get('/html/getDropdownList', 'FORM_CLASS', function (options) {
		HtmlUtil.singleSelect('select#formClass', options);
	}, ajaxSetting);
	<%-- 取得工作組別清單 --%>
...
function initEvent () {
	$('select#formClass').change(function () {
		let clazz = $(this).val();
		
		<%-- 先清空表單查詢欄位 --%>
		$("#fieldsTable tr").remove(); 
...
function search (record) {
	...
	if (allowFormClazzes.indexOf($("#formClass").val()) >= 0) {
		dataTable = TableUtil.paging('#dataTable', dynamicOpt(searchForm));
	} else {
		dataTable = TableUtil.paging('#dataTable', defaultOpt(searchForm));
	}
	
	SessionUtil.record('formSearch', searchForm);
	
	if (record) {
		$('select#formClass').val(record.formClass).change();
	}
}
...
function initColumn(){
	var column = '<tr>';
	column += '<th width="10%">' + i18nformId + '</th>';
	column += '<td width="85%"><input id="formId" name="formId" type="text" value="" style="width: 15rem;" /></td>';
	column += '</tr>';
...
function genTableColDefs (columnIdx, columns, item) {
	...
	info["render"] = function (data, type, row) {
		...
		} else if (id === 'associationForm') {// 關聯表單加入連結
			if (data) {
				...
				$.each(formIds, function (i, formId) {
					if (formId) {
						data += i != 0 ? '<br>' : '';
						data += '<a href="javascript: toForm(\'' + formId + '\')">' + formId + '</a>';
					}
				});
			}
		} else if (data && id === 'countersigneds') {
			let countersigneds = data.split(',');
			data = '';
			$.each(countersigneds, function (i, c) {
				if (c) {
					data += i != 0 ? '<br>' : '';
					data += c;
				}
			});
		} else if (id === 'cType') {// DC科
			let cType = $('#cType').val();
		
			if (!cType) {// 全部
				data = row.cType ? '/' + row.cType : '';
				data += row.cDataType ? '/' + row.cDataType : '';
				data += row.cbFormId ? '/<s:message code="form.title.tabBatch" text="批次" />' : '';
				data = data.substring(1, data.length);
			} else {
				if (cType == 'BATCH' && row.cbFormId) {
					data = '<s:message code="form.title.tabBatch" text="批次" />';
				} else if (cType == 'DB' && row.cType) {
					data = row.cType;
				} else {
					data = row.cDataType;
				}
...
function getCClass(id){
	if (id) {
		...
		HtmlUtil.emptySelect("#cComponent");
	}
}

<%-- 取得組態元件類別  --%>
function getKnowledge2 (id) {
	if (id) {
		SendUtil.get('/html/getSubDropdownList', id, function (options) {
			HtmlUtil.singleSelect('select#knowledge2', options);
		}, ajaxSetting);
	} else {
		HtmlUtil.emptySelect("#knowledge2");
	}
}

<%-- 取得組態元件 --%>
function getCComponent(id){
	if (id) {
		SendUtil.get('/html/getSubDropdownList', id, function (options) {
			HtmlUtil.singleSelect('select#cComponent', options);
		}, ajaxSetting);
	} else {
		HtmlUtil.emptySelect("#cComponent");
	}
