				...
				<tr align="center">
					...
					<c:if test="${isCountersign == 'Y'}">
						...
						<th style='display:none'>
							<c:forEach var="apply" items="${formProcessManagmentFormVo.applyProcessList}" varStatus="status" >
								<c:if test="${'Y' eq apply.isParallel}">
									<input id='parallels' name='parallels' value='${apply.parallels}' />
								</c:if>
							</c:forEach>
						</th>
					</c:if>
					...
					<c:if test="${isCountersign != 'Y'}">
						<th>
							<s:message code="form.process.managment.column.is.create.q.c.issue" text="可否產生問題會辦單	"/>
						</th>
					</c:if>
					<th>
					...
					<th>
						<s:message code="form.process.managment.add.page.review.process.is.modify.column.data.1" text="可修改" />
					</th>
...
function saveProcess() {
	...
	$.each(applyProcess,function() {
		...
		let isCreateCIssue = 						$(this).find("#isCreateCIssue").prop("checked");//可否產生問題會辦單
	...
	//Update
	SendUtil.post('/formProcessManagmentQuestion/updateFormProcess', mainParams, function (data) {
