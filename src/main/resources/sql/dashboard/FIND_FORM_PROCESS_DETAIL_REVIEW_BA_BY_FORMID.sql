...
FROM   (SELECT 
        ...
        FROM  (SELECT * 
               FROM  (SELECT f.*, 
                      ...
                      WHERE  f.verifytype = 'REVIEW' 
                             AND f.completetime IS NULL) pvl 
               WHERE  pvl.newest = 1 
                      AND pvl.formtype = 'BA') form_log 
              INNER JOIN(SELECT * 
                         FROM   (SELECT * 
                                 FROM   form fc 
       ...
       LEFT JOIN(SELECT pro.*, 
                 ...
                 FROM   (SELECT detailid, 
                                processorder, 
                                groupid 
                         FROM   form_process_detail_review_ba) pro 
...
JOIN
        FORM_INFO_BA_DETAIL detail
ON
        temp.FormId = detail.FormId
JOIN
        FORM_INFO_DATE formDate
ON
...
WHERE  EXISTS(SELECT 1 
              FROM   form_process_detail_review_ba PD 
