 ...
 */
@Component
public class QCValidateColHelper extends BaseFormValidateHelper implements FormColumnValidate {
    ...
    public List<String> initColumns (CommonFormVO vo) {
        return Arrays.asList(UnEditableEnum.Q_C.columns());
    }
    ...
    public List<String> proposeColumns (CommonFormVO vo, SysUserVO userInfo) {
        ...
        if (isCreator(vo, userInfo)) {
            columns.addAll(
                    Arrays.asList(UnEditableEnum.Q_C.columns()));
        } else {
    ...
    public List<String> disableAllColumns (CommonFormVO vo) {
        List<String> allColumns = allColumns();
        allColumns.add(BaseColEnum.SPC_GROUPS);
    ...
    public List<String> applyColumns (CommonFormVO vo, SysUserVO userInfo) {
        ...
        if (isVerifyAcceptable) {
            ...
            if (isApplyLevelOne(vo, userInfo)) {
                columns.addAll(Arrays.asList(EditOneEnum.Q_C.columns()));
            }
            
            if (!isApprover(vo)) {
                columns.add(BaseColEnum.SYSTEM);
                columns.add(BaseColEnum.SYSTEM_ID);
                columns.add(BaseColEnum.SYSTEM_DIALOG);
                columns.add(BaseColEnum.ASSET_GROUP);
                columns.add(BaseColEnum.UNIT_ID_2);
                ...
                if (isModifyColumnData) {
                    columns.addAll(Arrays.asList(EditableEnum.Q_C.columns()));
                }
            }
        }
