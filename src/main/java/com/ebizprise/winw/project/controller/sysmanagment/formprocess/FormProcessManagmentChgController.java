 ...
 */
@Controller
@RequestMapping("/formProcessManagmentChg")
public class FormProcessManagmentChgController extends BaseFormProcessManagmentController {
    
    //private static final Logger logger = LoggerFactory.getLogger(FormProcessManagmentChgController.class);
    ...
    public ResponseEntity<Map<String,Object>> insertFormProcess(@RequestBody FormProcessManagmentChgFormVO formVo) {
        return ResponseEntity.ok(getResultMessage(formProcessChgService.insertFormProcess(formVo)));
    }
    ...
    public ResponseEntity<Map<String,Object>> updateFormProcess(@RequestBody FormProcessManagmentChgFormVO formVo) {
        //若為啟用狀態,則無法被更新
        if(StringConstant.SHORT_YES.equalsIgnoreCase(formVo.getIsEnable())) {
            return ResponseEntity.ok(getCannotUpdateMessage());
        }
        
        return ResponseEntity.ok(getResultMessage(formProcessChgService.updateFormProcess(formVo)));
    }
